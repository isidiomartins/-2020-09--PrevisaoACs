...
#' @param type tipo
#' @param lags lag
#' @param selectlags  tipo
#'
#' @return Uma lista
#' @export
ur.df.modified <- function (y, type = c("none", "drift", "trend"),
          lags = 0, selectlags = c("Fixed", "AIC", "BIC")){
  ...
  # Isidio: Foi adicionado o objeto ur.df.modified, onde a função new() foi substituída por list().
  ur.df.modified <- list(y = y, model = type, cval = cvals, lags = lag,
                         teststat = teststat, testreg = testreg, res = res,
                         test.name = "Augmented Dickey-Fuller Test",
                         result = result) # Adiciona como
}
